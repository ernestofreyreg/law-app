...
export function MatterFormModal({
  ...
  useEffect(() => {
    if (matterData && isEditMode) {
      setFormData({
        ...
        closeDate: matterData.closeDate
          ? new Date(matterData.closeDate).toISOString().split("T")[0]
          : "",
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.status === "closed" && !formData.closeDate) {
      setValidationError(
        "A close date is required when status is set to Closed"
      );
      return false;
    }
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label
                  htmlFor="closeDate"
                  className={formData.status === "closed" ? "font-medium" : ""}
                >
                ...
                <Input
                  ...
                  className={
                    formData.status === "closed" && !formData.closeDate
                      ? "border-red-500"
                      : ""
                  }
                />
